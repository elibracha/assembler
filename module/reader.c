...
void assemble(char *file) {
	...
	while (!feof(fd)) {
		...
		case OPCODE:
			...
			break;
		case OPRAND:
			handle_operand(&size, ch, &line, &has_label, &cmd, &state,
					&sub_state);
			break;
		case COMMENT:
...
void handle_opcode(signed short int *size, char ch, signed short int *line,
		_Bool *has_label, char **commend, enum status *pstatus) {
...
void handle_operand(signed short int *size, char ch, signed short int *line,
		_Bool *has_label, char **commend, enum status *pstatus,
		enum sub_status *psub_status) {
	int result = 0;
	if (before_param || after_param) {
		if (handle_line(size, ch, line, has_label, &(*commend),
				pstatus) || ch == SPACE || ch == TAB) {
			return;
		}
	} else if ((result = handle_line(size, ch, line, has_label, &(*commend),
			pstatus)) || ch == SPACE || ch == TAB) {
		if (result)
			comma_error = 0;
		else {
...
_Bool handle_line(signed short int *size, char ch, signed short int *line,
		_Bool *label, char **commend, enum status *pstatus) {
