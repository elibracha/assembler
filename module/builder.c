...
void data_handler(char *label, char **operands, int params) {
	...
	while (params) { //building the clean string number without any unknown chars.
		...
		if (get_head_data() == NULL) {
			if (flag) {
				insert_first_data(DC++,
						plus_to_minus(convert_10bits_to_2(number, 1))); // handling first data and also minus number.
			} else {
			...
			if (flag) {
				insert_last_data(DC++,
						plus_to_minus(convert_10bits_to_2(number, 1))); // handling minus number.
			} else {
...
void string_handler(char *label, char **operands, int params) {
	...
	for (i = 0; i < strlen(*operands); ++i) { // running on each char in the string to asign a "byte".
		...
		if (get_head_data() == NULL) { // defualt add to a char after passing all tests above for valid char.
			insert_first_data(DC++,
					convert_10bits_to_2((int) *((*operands) + i), 1));
		} else {
			insert_last_data(DC++,
					convert_10bits_to_2((int) *((*operands) + i), 1));
		}
...
_Bool check_Addressing_1(char *operand, int line, _Bool num_operand) {
	...
	if (check_Addressing_0(operand, line, &temp_result)) { // checking that no data variable gets number representation.
		if (num_operand == 0) {
			if (cround != 2) {
				printf(MISSPLACED_ADDRESSING, 2, 1, "Immediate Number", line); // case first operand is assigned with a number.
				ERRORS++;
			}
		} else {
			printf(MISSPLACED_ADDRESSING, 1, 1, "Immediate Number", line); // case second operand is assigned with a number.
			ERRORS++;
		}
		return 0;
	...
	for (i = 0; i < strlen(operand); ++i) {
		...
		} else if ((*(operand + i) < 97 && *(operand + i) > 90)
				|| *(operand + i) > 120
				||
...
void handle_cmd(char *label, char **operands, int line, method *md, int args,
		int round) {
	commend cmd;
...
void handle_round2_c2(int val_op1, int val_op2, char **operands, int line,
		int param) {
	char *m1 = NULL;
